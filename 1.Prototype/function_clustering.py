...
import format_code
import hunkmap
from codebert import codebert_sim
import joern
...
def get_clustring(repo_path, pre_commit, post_commit, cveid, trigger_point = None):
    ...
    if not os.path.exists(f"{local_cache_dir}/sub_call.json"): 
        try:
            ...
            pre_project, post_project = pre_post_projects
            pre_function_dict = {}
            for method_signature in callgraph["pre"]["points"]:
                pre_function_dict[method_signature] = pre_project.get_method(method_signature)
            post_function_dict = {}
            for method_signature in callgraph["post"]["points"]:
                post_function_dict[method_signature] = post_project.get_method(method_signature)
            ...
            ## codebert enhance 
            critical_funcs = codebert_sim(critical_funcs, pre_function_dict, post_function_dict)
