...
from recovery import CodeMapper, checking_recovery
from parse_commit import get_file_from_commit
...
def recovery(meta, local_cache_dir, cveid):
    ...
    for info in method_info:
        ...
        # Create code mapper for semantic equivalence
        mapper = CodeMapper(pb, pb_formatted)
        
        # Transform and recover code
        pc_recovered = mapper.transform_to_original(pc)
        recovered_code = checking_recovery(pa, pc_recovered, pb.split("\n"), info['add'])
        
        # Generate diff
        diff_per = gitdiff(pa, recovered_code)
        diff += diff_per
