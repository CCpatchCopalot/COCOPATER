...
def split_define_assign(st, code: str, language: Language):
    ...
    # return st, astyle(code_bytes.decode()), language
    return st, code_bytes.decode(), language
...
def code_transformation(codes: str, language: Language):
    ...
    for st, ed in line_interval:
        # print("***********************", st_before, ed_before, st,ed,"******************")
        if st_before != 1 and ed_before + 1 != st:
            # print(ed_before+1, st-1)
            codes_split.append(
    ...
    results = cpu_heater.multithreads(
        operator_extraction, codes_split, max_workers=512, show_progress=False
    )
    # for st, res, language in results:
    #     st1, res1, language1 = merge_control_blocks(st, res, language)
    results = cpu_heater.multithreads(
    ...
    # # results = cpu_heater.multithreads(operator_extraction, results, max_workers=512, show_progress=False)
    # results = cpu_heater.multithreads(move_definition, results, max_workers=512, show_progress=True)
    codes = ""
    ...
    while True:
        ...
        for i, node in enumerate(nodes):
            ...
            try:
                ...
                print(nodes[i - 1].text.decode(), nodes[i].text.decode(), lines, code)
                print(pre_if_block_end, if_block_start, len(lines))
            ...
            if if_continuous:                
                ...
                # the two if blocks are the same
...
def modify_if_else(st:int, code:str, language:Language):
    ...
    while True:
        ...
        for node in nodes:
            ...
            condition = else_clause.child(1).child_by_field_name("condition") # type: ignore
            if condition is None:
                continue
