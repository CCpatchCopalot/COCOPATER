...
def get_ast_list (code: str):
    if code == "":
        return ""
    parser = ASTParser(code, Language.C)
    ast_list = []
    for node in parser.traverse_tree():
        ast_list.append(f"{node.type} {node.text}")
    return " ".join(ast_list)
...
def get_patch_hunks(code1: str, code2: str) -> list[Hunk]:
    ...
    for a_hunk, b_hunk in modify_hunks_map.items():
        hunk = ModHunk(HunkType.MOD, a_hunk[0], a_hunk[1], b_hunk[0], b_hunk[1], "\n".join(code1_lines[a_hunk[0] - 1:a_hunk[1]]),
                       "\n".join(code2_lines[b_hunk[0] - 1:b_hunk[1]]), gitdiff("\n".join(code1_lines[a_hunk[0] - 1:a_hunk[1]]), "\n".join(code2_lines[b_hunk[0] - 1:b_hunk[1]])), gitdiff(get_ast_list("\n".join(code1_lines[a_hunk[0] - 1:a_hunk[1]])), get_ast_list("\n".join(code2_lines[b_hunk[0] - 1:b_hunk[1]]))))
    ...
    for add_hunk in addLinesGroup:
        ...
        if (first_line, last_line) not in modify_hunks_map.values():
            ...
            hunk_list.append(AddHunk(HunkType.ADD, first_line, last_line,
                             "\n".join(code2_lines[first_line - 1:last_line]), insert_line, gitdiff("", "\n".join(code2_lines[first_line - 1:last_line])), gitdiff(get_ast_list(""), get_ast_list("\n".join(code2_lines[first_line - 1:last_line])))))
    for del_hunk in delLinesGroup:
        ...
        if (first_line, last_line) not in modify_hunks_map.keys():
            hunk_list.append(DelHunk(HunkType.DEL, first_line, last_line,
                             "\n".join(code1_lines[first_line - 1:last_line]), gitdiff("\n".join(code1_lines[first_line - 1:last_line]), ""), gitdiff(get_ast_list("\n".join(code1_lines[first_line - 1:last_line])), get_ast_list(""))))
    return hunk_list
