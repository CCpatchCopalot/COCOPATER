...
class Patch:
    def __init__(self, repo_path: str, pre_commit_id: str, post_commit_id: str, language: Language):
        ...
        for diff in diffs:
            ...
            if diff.change_type == "A":
                ...
                diff_and_sc = {
                    "diff": gitdiff("", format.format(self.post_files[diff.b_path], language, del_comment=True, del_linebreak=True, transform=False)),
                    "source_code_before": "",
                    "source_code": format.format(self.post_files[diff.b_path], language, del_comment=True, del_linebreak=True, transform=False),
                }
                self.patch_content += diff_and_sc["diff"]
                modification = Modification(
                    diff.a_path, diff.b_path, ModificationType.ADD, diff_and_sc
                )
                self.add_files.append(modification)
                self.diffs.append(modification)
                ...
                diff_and_sc = {
                    "diff": gitdiff(format.format(self.pre_files[diff.a_path], language,
                                                del_comment=True, del_linebreak=True, transform=False), ""),
                    "source_code_before": format.format(self.pre_files[diff.a_path], language,
                                                del_comment=True, del_linebreak=True, transform=False),
                    "source_code": "",
                }
                self.patch_content += diff_and_sc["diff"]
    ...
    def diff_hunks(self):
        ...
        for diff in self.diffs:
            hunks.extend(DiffBlob(diff, self.pre_project, self.post_project).hunks)
        return hunks
    ...
    def post_files(self):
        ...
        # print("post files", len(all_blob_contents))
        return all_blob_contents
    ...
    def get_all_blobs(self, root_tree):
        ...
        # results = cpu_heater.multithreads(self.get_one_blob, worker_list, max_workers=65535, show_progress=True)
        # for res in results:
        #     all_blob_contents.update(res)
        return all_blob_contents
    ...
    def change_method_map_dict(self):
        ...
        for file in self.modify_files:
            ...
            if pre_file is None:
                logger.warning(f"❌ pre_file does not exist: {path}")
                continue
            if post_file is None:
                logger.warning(f"❌ post_file does not exist: {path}")
                continue
            # 
    ...
    def avarage_method_change(self) -> float:
        ...
        for method_sig in changed_methods:
            ...
            if pre_method is None or post_method is None:
                logger.warning(f"❌ Method does not exist: {method_sig}")
                continue
    ...
    def changed_methods(self) -> set[str]:
        ...
        for file in self.modify_files:
            ...
            for method in pre_file.methods:
                # if method.signature != "tools/tiffcrop.c#readContigTilesIntoBuffer":
                #     continue
                if (
                    method.signature
                    in self.added_methods_set | self.deleted_methods_set
                    ...
                    # if method_line_set == delete_lines:
                    continue  # 
                method_line_set = set(
                    range(method.body_start_line, method.body_end_line + 1)
                )
        ...
        for method in changed_methods.copy():
            ...
            if pre_method is None or post_method is None:
                logger.warning(f"❌ Method does not exist: {method}")
                continue
        ...
        for pre_method_name in change_method_map_dict.keys():
            ...
            if pre_method is None or post_method is None:
                logger.warning(f"❌ Method does not exist: {pre_method_name}")
                continue
            pre_method.counterpart = post_method
            post_method.counterpart = pre_method
    ...
    def format(self, FilePath, ispre):
        # fp = open(os.path.join(root, file), encoding='utf-8', errors='ignore')
        # codes = fp.read()
        # fp.close()
        ...
        codefile = CodeFile(FilePath, codes, isformat=True, language = self.language)
        return codefile
    ...
    def pre_bfs_search_files(self, filepath, methodname, step=0):
        ...
        # fp = open(os.path.join(self.repo_path, filepath), "r")
        ...
        for include in includes:
            ...
            if os.path.join(prefix, include_name) in self.pre_files:
                # if os.path.isfile(os.path.join(self.repo_path, prefix, include_name)) and not os.path.islink(os.path.join(self.repo_path, prefix, include_name)):
                for suffix in suffix_list:
                    if (
                        os.path.join(prefix, include_name.replace(".h", suffix))
                        in self.pre_files
                    ):
                        # if os.path.isfile(os.path.join(self.repo_path, prefix, include_name.replace(".h", suffix))) and not os.path.islink(os.path.join(self.repo_path, prefix, include_name.replace(".h", suffix))):
                ...
                if len(callees) == 0:
                    break
            # elif os.path.isfile(os.path.join(self.repo_path, prefix, "include", include_name)) and not os.path.islink(os.path.join(self.repo_path, prefix, "include", include_name)):
                    ...
                    if os.path.isfile(
                        ...
                        for method_name, method_contents in file_contents:
                            if method_name in callees:
                                ...
                                callees.remove(method_name)
                    # elif os.path.isfile(os.path.join(self.repo_path, prefix, "src", include_name.replace(".h", suffix))) and not os.path.islink(os.path.join(self.repo_path, prefix, "src", include_name.replace(".h", suffix))):
                    elif (
    ...
    @cached_property
    def pre_analysis_files(self):  # -> list[CodeFile]:
        """
         
        """
        results = []

        # subprocess.run(["git", "checkout", "-f", self.commit_id + "~1"], cwd=f'{self.repo_path}', check=True, text=True, capture_output=True)
        # print(self.changed_methods)
        # if len(self.changed_methods) > 10:
        #     return []
        for method_sig in self.changed_methods:
            # logger.info(f"🔍 {method_sig}")
            self.pre_bfs_search_files(
    ...
    @cached_property
    def post_analysis_files(self):  # -> list[CodeFile]:
        """
        ...
        # if len(self.changed_methods) > 10:
        #     return []
        # subprocess.run(["git", "checkout", "-f", self.commit_id], cwd=f'{self.repo_path}', check=True, text=True, capture_output=True)
        for method_sig in self.changed_methods:
            self.post_bfs_search_files(
                method_sig.split("#")[0], method_sig.split("#")[1]
            )
...
class DiffBlob:
    def __init__(self, modification: Modification, pre_project: Project, post_project: Project):
        self.a_path = modification.old_path
        self.b_path = modification.new_path
        self.change_type = modification.change_type
        self.a_blob_content: str | None = None
        self.b_blob_content: str | None = None
        self.hunks: list[Hunk] = []

        if modification.old_path is None and modification.new_path is not None:
            # 
            self.change_type = "A"
        elif modification.old_path is not None and modification.new_path is None:
            
            self.change_type = "D"
        elif (
            modification.old_path is not None
            and modification.new_path is not None
        ...
        if self.a_path is not None:
            self.a_blob_content = pre_project.get_file(self.a_path).code
        if self.b_path is not None:
            self.b_blob_content = post_project.get_file(self.b_path).code

        if self.change_type == "M" or self.change_type == "C":
            # print()
            self.hunks = self.parse_hunks(modification.diff)  # type: ignore
    ...
    def parse_hunks(self, diff: str) -> list:
        ...
        for hc in hunks_content:
            hunk = Hunk(hc, self.a_path)
            hunks.append(hunk)
        return hunks


class Hunk:
    def __init__(self, hunk_content: str, file_name: str):
        ...
        lineinfo = re.match(r"@@ -(\d+),(\d+) \+(\d+),(\d+) @@", hunk_content).groups()  # type: ignore
