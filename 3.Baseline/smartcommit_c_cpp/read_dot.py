...
# Handle the error of escape character in the dot file
...
def correct_syntax_error(dot_file, line_number, token):
    ...
    # find the fist index of the token
    index = None
    for match in re.finditer(pattern, target_line):
        if match.group(1):  # find prefix
            continue
        if match.group(3):  # not find prefix
            index = match.start(3)  # get the index of the token
            break
...
if __name__ == "__main__":
    read_dot_in_fix_mode(
        "/Users/huangyh/malware-dynamic/joern-workspace/nodetree_test/cpg/export.dot"
    )
